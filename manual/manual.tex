...
\lstset{belowskip=-0.7\baselineskip}
...
\verb!/usr/share/tascar/matlab!   & tools for GNU Octave and MATLAB \\
\verb!/usr/share/tascar/python!   & tools for python/blender        \\
\end{tabular}
\medskip
\hrule
\caption{List of relevant \tascar{} directories on Linux installations\index{directories}.}\label{tab:instdir}
\end{table}
...
TASCAR relies heavily on the jack audio connection kit
(\url{http://jackaudio.org}). It is necessary to start jack before
loading a session into TASCAR. TASCAR will attempt to start
\verb!qjackctl! if the jack server is not running. This behaviour can
be disabled by adding an entry to the GUI section of the configuration
file (see Section \ref{sec:globalconfig} for details):
...
Jack port names in TASCAR are treated as POSIX regular expressions
\citep{posixregexp}. If the \verb!^! or \verb!$! anchors are not
present at the beginning or end of an expression, they will be added
at the beginning and end of the expression to achieve proper full name
matching. Please note that some characters such as \verb!.[?()! (and a
few others) have special meaning as regular expressions and must be
quoted for a correct match.

When using TASCAR with jackd1, memory locking may fail, resulting in
the TASCAR process being killed. If you see this behaviour, either run
jack without memory locking (using the \verb!-m! flag), or use jackd2
instead.

An acoustic environment can be loaded either from the command line by
specifying the filename, or from the File menu in the main window. If
loading a session file fails for some reason, it may be helpful to run
it from the command line to see additional information.

TASCAR uses SI units unless otherwise specified. Developers of new
...
The majority of options in \tascar{} can be controlled remotely via the Open Sound Control (OSC) protocol. An OSC message comprises one or more numeric values or strings. Each message can be transmitted to an OSC path (e.g., \verb!/scene/object/gain!) on an OSC server (e.g., a UDP server listening on port 9877 at host \verb!localhost!). \tascar{} supports both UDP and TCP transport layers for the receipt of OSC messages. The ``oscserver'' module can be used to combine multiple OSC transport methods or multiple ports (for further details, please refer to section \ref{sec:oscserver}). Additionally, a GNU Octave/MATLAB remote control tool is available (for further details, please refer to section \ref{sec:sendosc}).
...
When a message to \verb!/sendvarsto! arrives, first an empty message is sent to \verb!<path>/begin!.
%
...
%
If you receive truncated or missing OSC messages, switch to TCP transport.
...
Here is a minimal \verb!.tosc! file example:
\begin{verbatim}
# Set gain of source 1 to 0.5
/scene/objects/1/gain 0.5

# Wait 2 seconds before next message
,2
/scene/objects/1/gain 1.0

# Run nested script
<my_nested_script.tosc
\end{verbatim}

\subsection{Optimization of the operating system for audio processing}

To achieve the best performance in delay-critical setups, it is recommended that you optimise the operating system. Below, you will find some tips for optimising Debian-based Linux operating systems. You can use the script \verb!tascar_assertrt! to test the current system. This script tests for real-time optimisations and reports warnings if any are missing from the current system.

\paragraph{Hint:}

Add a \elem{system} module to your session file with with script, to test for optimisations upon loading the session:
...
<session>
  <scene> ... </scene>
  <modules>
    <system command="tascar_assertrt"/>
  </modules>
</session>
\end{lstlisting}

\subsubsection*{User and group settings}
...
\end{verbatim}
Or, to add the current user to the audio group:
\begin{verbatim}
...
If you are aiming for low-latency processing, you should further optimize your system by installing a realtime (if available) or low-latency kernel and an IRQ priority management tool:
\begin{verbatim}
sudo apt install linux-realtime rtirq-init
\end{verbatim}
Alternatively, if a realtime kernel is not available, type:
\begin{verbatim}
sudo apt install linux-lowlatency rtirq-init
\end{verbatim}

When starting Jack, choose a real-time priority slightly lower than the interrupt handler priority of the selected sound card. This prevents the audio thread from preempting the sound card's interrupt handler, which could result in buffer underruns. You can check this with the \verb!tascar_testrtprio! tool in the console. A sample output may look like the following:
\begin{lstlisting}[numbers=none]
       CPU0   CPU1   CPU2    CPU3       
...
If cpufreq-selector is not available, install it via sudo apt install cpufrequtils.
If this doesn't work please use the CPU frequency scaling indicators of your desktop manager, or with
\begin{verbatim}
echo "performance" | sudo tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor
\end{verbatim}

\subsubsection*{Virtual CPUs}

Most modern computers with Intel CPUs can use their physical CPUs to
serve as multiple logical CPUs. This can potentially improve
performance by splitting the load among independent instruction
sets. However, this disrupts real-time scheduling because multiple
logical CPUs share the same physical core. This means that
non-real-time scheduled code can run in parallel with real-time
scheduled code on the same physical core and therefore block real-time
code. Therefore, it is recommended that Hyper-Threading technology is
disabled on computers used for delay-critical audio processing.
...
Virtual Acoustic Scenes are created using the XML scene definition file format\index{file format}. The \tascar{} scene definition (\verb!.tsc! file) is a text XML file in which the user specifies all the details about the scene using various commands, XML elements and their attributes. An example of a scene definition is shown below (example file \verb!example_basic.tsc!):
...
\elem{session} is the root element of any TASCAR session file. It
...
frequencies, a cardioid-like radiation pattern is achieved, see Figure
\ref{fig:radiation_cardioidmod}.

\begin{figure}[htb]
  \centering
  \fbox{\includegraphics[width=0.8\textwidth]{cardioidmod.pdf}}
  \caption{Radiation pattern of {\tt cardioidmod} source directivity with default parameters.}
  \label{fig:radiation_cardioidmod}
\end{figure}
...
\subsubsection{Source directivity ``head''}


\begin{figure}[htb]
  \centering
  \fbox{%
    \includegraphics[width=0.4\textwidth]{radiation_head_transverse.pdf}%
    \includegraphics[width=0.4\textwidth]{radiation_head_sagittal.pdf}%
  }
  \caption{Radiation pattern of {\tt head} source directivity with
    default parameters, in the transverse plane (left panel) and the
    sagittal plane (right panel).}
  \label{fig:radiation_head}
\end{figure}


A source radiation pattern similar to a human head is approximated,
roughly based on data by \citet{Poerschmann2021}. The resulting
radiation pattern is shown in Figure \ref{fig:radiation_head}.

\input{tabsoundhead.tex}

\input{oscdoc_tascarsource_head.tex}
...
\subsection{Loudspeaker-based receiver types}\label{sec:speaker}\index{loudspeaker}
...
The loudspeaker layout of loudspeaker-based receiver types can be defined in a separate layout file specified in the \attr{layout} attribute, or in a list of \elem{speaker} elements within the receiver definition. Each of the \elem{speaker} entries has the following attributes:

\input{tabspeaker.tex}

In addition to regular broadband loudspeakers, a number of subwoofers\index{subwoofer} can be defined using \elem{sub} elements with the same attributes as in the \elem{speaker} element. When subwoofers are defined, an IIR crossover filter with 24~dB/octave is applied to all signals. The subwoofer signals are spatially mapped from the broadband loudspeaker positions to the subwoofer positions using a modified DBAP \citep{Lossius2009} method.
...
As the calibration values may depend on the rendering method and its parameters, the \attr{calibfor} attribute must be set to the correct value for the session file in which the loudspeaker layout is used. These values can be queried using the command line tool \verb!tascar_getcalibfor! which prints out the correct value for each loudspeaker-based receiver type used in a session file.

A simple example of a loudspeaker layout file is shown in Example \ref{spk:nsp}.
...
Obstacles are polygonal meshes that can absorb sound and cause diffraction at their edges. The diffraction pattern is only an approximate representation. Currently, obstacles only affect the sound path from a primary sound source to a receiver.
...
For a calibration of loudspeaker layouts, it is recommended to use the
...
\subsection{Calibrating loudspeaker layouts with {\tt tascar\_spkcalib}}\label{sec:spkcalib}

All loudspeaker-based rendering methods (e.g., those depending on a loudspeaker layout file) should result in identical levels at the listening positions for virtual sound sources from the directions of the loudspeakers (the levels of interpolated virtual sources may differ due to differences in the rendering method).
%
...
Spectral correction can be performed using parametric equalizers or FIR filters with a minimum-phase filter response. When parametric equalizers are used, each filter stage (biquad) is tuned to adjust the center frequency, bandwidth (Q), and gain in order to compensate for deviations in the loudspeaker and room response. A parametric equalizer is applied to each loudspeaker to flatten the frequency response at the measurement microphone position and ensure consistent amplitude response across the array. The equalizer coefficients are computed by minimizing the difference between the measured and target (flat) frequency responses using a least-squares approach. If FIR filters are used for equalization, the difference between the measured and target frequency responses is used to calculate a minimum-phase compensation filter. This filter is implemented as a partitioned convolution with constant partition sizes and is truncated at the given length.

If the wizard is started without specifying a layout file, a page for selecting a layout opens first. In the next step the calibration parameters can be revised (see Figure \ref{fig:spkcalib1}). ``fmin'' and ``fmax'' determine the frequency range of the calibration stimulus. Within the given frequency range a $1/f$ characteristic is used. ``duration'' defines the duration of the level measurement, ``prewait'' the waiting time between switching on the test stimulus and starting the level measurement. The target level is specified in the ``reference level'' field. The other fields refer to the frequency response correction: ``bands per octave'' defines the frequency resolution of the analysis filter bank. ``overlap in bands'' designates a spectral smoothing over adjacent frequency bands, e.g. to minimize the influence of notches. ``max number of filter stages'' is the maximum number of equalizer stages, where each stage is realized by a parametric equalizer (biquad filter). ``max number of FIR filter coefficients'' defines the length of the FIR filters to be used. Only one method of loudspeaker equalization method can be used, therefore either the number of equalizer stages or the filter length must be set to zero.
...
\begin{figure}[htb]
    ...
    \caption{Equalization of loudspeakers, with spectral correction. The center frequencies of the equalizer stages are indicated with blue markers. The thin gray line indicates the frequency response without spectral correction, the thick red line with spectral correction.}
    \label{fig:spkcalib3}
\end{figure}
...
\section{Interfacing from GNU/Octave and MATLAB}

For the interface between \tascar{} and GNU/Octave or MATLAB a set of
...
function help in GNU Octave/MATLAB.
...
An example of the usage of this (and other GNU Octave/MATLAB functions) can be
...
format.
%
User can write such a file on his own or create it using GNU Octave/MATLAB
function \verb!generate_scene!.
%
This function can generate a simple scene with one loudspeaker-based
...
want to start performing some measurements using GNU Octave/MATLAB.
%
...
%
For example, in GNU Octave/MATLAB it can be done by using the provided
...
\subsection{\tt tascar\_xml\_*}\label{sec:tascarxml}

A set of functions to manipulate XML files are provided. For detailes,
see the help comment of the functions, or the GNU/Octave or Matlab
tutorial on github.
