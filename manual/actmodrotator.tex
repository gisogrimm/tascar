...
(\attr{mode="0"}, default), {\em sigmoid} (\attr{mode="1"}), {\em
  cosine} (\attr{mode="2"}), {\em free} (\attr{mode="3"}), and {\em
  linear (limited)} (\attr{mode="4"}).

\input{modtabtex/tabrotator.tex}
...
\paragraph{Linear (limited) rotation}

\begin{lstlisting}[numbers=none]
  <rotator mode="4" t0="1" t1="4" phi0="0" phi1="120" actor="/*/out"/>
\end{lstlisting}

\begin{equation}
O_z = \left\{\begin{array}{ll}
\varphi_0 & t<t_0\\
\varphi_0+(\varphi_1-\varphi_0)(\frac{t-t_0}{t_1-t_0}) & t_0\le t \le t_1\\
\varphi_1 & t_1 < t
\end{array}\right.
\end{equation}
